...
for (i=1:2),
  for (j=1:numAlleles),
    for (k=1:numAlleles),
      genotype = allelesToGenotypes(j, k);
      pos = AssignmentToIndex([i, j, k], phenotypeFactor.card);
      if (i==1),
        phenotypeFactor.val(pos) = alphaList(genotype);
      else
        phenotypeFactor.val(pos) = 1 - alphaList(genotype);
      end
    end
  end
end
