...
I = IndexToAssignment(1:prod(genotypeFactor.card), genotypeFactor.card);
for (i=1:length(I)),
  childGenotype = I(i, 1);
  parent1Genotype = I(i, 2);
  parent2Genotype = I(i, 3);
  ...
  for (n=1:2),
    for (m=1:2),
      if (childAlleles(1) == parent1Alleles(n) && childAlleles(2) == parent2Alleles(m)
        || childAlleles(2) == parent1Alleles(n) && childAlleles(1) == parent2Alleles(m)),
