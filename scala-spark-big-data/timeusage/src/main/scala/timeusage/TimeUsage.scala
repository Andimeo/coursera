...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsPrefixes = Array("t01", "t03", "t11", "t1801", "t1803")
    val workingActivitiesPrefixes = Array("t05", "t1805")
    val groups = columnNames.groupBy(name => {
      if (primaryNeedsPrefixes.map(name.startsWith(_)).fold(false)(_ || _))
        "primaryNeeds"
      else if (workingActivitiesPrefixes
                 .map(name.startsWith(_))
                 .fold(false)(_ || _))
        "workingActivities"
      else
        "otherActivities"
    })
    def toColumn(name: String): Column = new Column(name)

    (
      groups("primaryNeeds").map(toColumn(_)),
      groups("workingActivities").map(toColumn(_)),
      groups("otherActivities").map(toColumn(_))
    )
  }
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"SELECT working, sex, age, ROUND(AVG(primaryNeeds), 1) as primaryNeeds," +
      s"ROUND(AVG(work), 1) as work, ROUND(AVG(other), 1) as other FROM $viewName " +
